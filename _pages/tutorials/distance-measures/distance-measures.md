...
* The Jaccard similarity is calculated by $$\text{Jaccard similarity} = {|\text{votes}_\text{country a} \bigcup \text{votes}_\text{country b}| \over |\text{votes}_\text{country a} \bigcap \text{vote}_\text{country b}|}$$, where $\bigcup$ stands for the votes where they agree, and the $\bigcap$ stands for all votes from both countries, irrespective whether they were similar or not. The $|$ surrounding the terms indicates they should be counted. The Jaccard similarity is turned into Jaccard dissimilarity by subtracting it from 1: $$\text{Jaccard dissimilarity} = 1-{|\text{votes}_\text{country a} \bigcup \text{votes}_\text{country b}| \over |\text{votes}_\text{country a} \bigcap \text{vote}_\text{country b}|}$$
